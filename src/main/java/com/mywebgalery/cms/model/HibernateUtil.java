...
public class HibernateUtil {
    ...
    private static Map<String, SessionFactory> _factories = new HashMap<String, SessionFactory>();
    ...
    static {
        ...
        try {
            ...
            log.info("hibernate.connection.url=" + configuration.getProperties().getProperty("hibernate.connection.url"));
            //log.info("sessionFactory.getAllClassMetadata()=" + sessionFactory.getAllClassMetadata());
        } catch (Throwable ex) {
     ...
     */
      ...
      * management, not with JNDI or any other external registry.
      */
      public static void rebuildSessionFactory(String connectionUrl) {
		log.debug("Rebuilding the SessionFactory from given URL.");
		if(_factories.get(connectionUrl)==null){
			Configuration cfg = HibernateUtil.getConfiguration().setProperty("hibernate.connection.url",connectionUrl);
			rebuildSessionFactory(cfg);
			_factories.put(connectionUrl, sessionFactory);
		} else {
			sessionFactory = _factories.get(connectionUrl);
		}
      }
